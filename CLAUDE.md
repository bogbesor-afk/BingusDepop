...
**2026-08-18 — visual redesign + one-click sales + shipping-aware restock (all live).** Benjamin is actively using the app with real data (`bogbesor@gmail.com`, item "Nike Set 1") — verify any future schema/RLS change is additive-only and never touches existing rows.

- **Theme:** flipped from dark to white background with a red (Depop-style) accent color for buttons/links/active nav. Sage green (`#8A9A5B`) and dark green (`#3B4A3B`) replace emerald/red everywhere money is shown as positive/negative (stat cards, chart lines) — deliberately *not* using red for "bad" since red is now the brand accent and would be confusing doubled up. `BingusDepop` wordmark enlarged in the nav and on login.
- **One-click sales:** "Log Sale" on the inventory card is no longer a form — it's `app/items/actions.ts`'s `logQuickSale()`, which logs 1 unit sold at the item's `sale_price_default` per click. This made `sale_price_default` a required field at item creation (previously optional). The old dedicated `/items/[id]/sell` page/form was removed entirely.
- **Shipping-aware restock:** added `items.lead_time_days` (migration `20260815010000_add_lead_time.sql`, default 14, asked for at item creation). `lib/dashboard.ts` now computes a real reorder point (`dailyVelocity × leadTimeDays`) instead of a flat 14-day threshold, and targets enough stock to cover lead time + 30 days.
- **Dashboard layout:** two-column — charts on the left, "General stats" + "Restock recommendations" on the right (`app/page.tsx`, `grid-cols-[minmax(0,1fr)_320px]` on `lg+`). Replaced the old "Top sellers by revenue" chart with a "Sales & projected gain" forecast chart (`ForecastChart` in `components/DashboardCharts.tsx`): actual revenue/profit as solid lines, plus a dashed linear-trend-projected-profit line 3 months forward, clearly labeled as an estimate, not a real forecast model. Small "Est. next month revenue/profit" stat chips sit directly under that chart.
- Verified with a throwaway test account locally and on production, math hand-checked (revenue, spend, profit, inventory value, reorder point, suggested quantity/cost, forecast line) — then deleted, leaving Benjamin's real account and item untouched.
